...
- **Slider Values:** Use `aria-valuetext` on range inputs when the numeric value (e.g., 2024) has specific semantic meaning (e.g., "Year 2024") to provide context to screen reader users, rather than relying solely on the raw number.
- **Interactive Legends:** Legend items that toggle data visibility must be keyboard accessible (`tabindex="0"`, `role="button"`) and provide clear focus indicators (e.g., rings or opacity changes) to match mouse hover behavior.
