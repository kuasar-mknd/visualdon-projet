...
- **Toggle Buttons vs. Radio Groups:** Do not convert simple exclusive toggle buttons (like Language or View Mode) to `role="radio"` unless implementing full arrow-key navigation. Using `role="group"` with `aria-pressed` is a more robust pattern for this codebase's button-based components, ensuring Tab navigation remains intuitive.
