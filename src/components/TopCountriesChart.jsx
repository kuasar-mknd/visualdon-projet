  ...
  useEffect(() => {
    ...
    update.select(".value-label")
        ...
        .tween("text", function(d) {
            // Optimization: Cache current value to avoid layout thrashing (parseFloat(textContent))
            const node = this;
            const current = node._currentValue || 0;
            const target = d[category] || 0;
            const i = interpolateNumber(current, target);
            return function(t) {
                const val = i(t);
