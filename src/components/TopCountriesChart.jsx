  ...
  useEffect(() => {
    ...
    update.select(".value-label")
        ...
        .tween("text", function(d) {
            // Optimization: Read from cached property to avoid layout thrashing (reading textContent)
            // which causes a style recalculation on every frame of the tween if we read what we just wrote.
            const current = this._currentValue || 0;
            const i = interpolateNumber(current, d[category] || 0);
            return function(t) {
                this._currentValue = i(t);
                this.textContent = this._currentValue.toFixed(1);
            };
        });
