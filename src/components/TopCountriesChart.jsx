...
const TopCountriesChart = ({ data, year, category, isPlaying, onCountrySelect, displayCategory }) => {
  ...
  // Optimization: Stable string dependency for translation effect
  // This prevents the effect from running on every animation frame when topData reference changes but countries are same
  const neededCountryCodesStr = useMemo(() =>
    topData.map(d => d["ISO 3166-1 alpha-3"]).join(','),
  [topData]);

  // Fetch translated country names when top countries change
  useEffect(() => {
    const codes = neededCountryCodesStr.split(',').filter(Boolean);
    const neededCodes = codes.filter(code => !translatedNames[code]);
    ...
    fetchTranslations();
  }, [neededCountryCodesStr, language, translatedNames]);
